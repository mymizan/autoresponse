...
class LogController extends AppBaseController
{
	...
	public function Query()
	{
		try
		{
			...
			if ($filter) $criteria->AddFilter(
				new CriteriaFilter('Id,From,TextMessage,Date'
				, '%'.$filter.'%')
			);
	...
	public function Create()
	{
		try
		{
			...
			$log->From = $this->SafeGetVal($json, 'from');
			$log->TextMessage = $this->SafeGetVal($json, 'textMessage');
			$log->Date = date('Y-m-d H:i:s',strtotime($this->SafeGetVal($json, 'date')));
	...
	public function Update()
	{
		try
		{
			...
			$log->From = $this->SafeGetVal($json, 'from', $log->From);
			$log->TextMessage = $this->SafeGetVal($json, 'textMessage', $log->TextMessage);
			$log->Date = date('Y-m-d H:i:s',strtotime($this->SafeGetVal($json, 'date', $log->Date)));
